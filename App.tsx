...
const App: React.FC = () => {
  ...
  return (
    <div className={`fixed inset-0 overflow-hidden transition-colors duration-500 font-jakarta ${colors.bg} ${colors.text}`}>
        ...
        <div ref={feedRef} onScroll={handleScroll} className="snap-y-container h-full w-full relative no-scrollbar">
          {facts.map((fact, i) => (
            <div key={fact.id} className="snap-item w-full h-full flex items-center justify-center p-8 pb-32 relative">
              {fact.isAd ? (
                ...
                <div className={`w-full max-w-sm h-[65%] p-10 rounded-[56px] shadow-sm relative bg-gradient-to-br ${theme === 'light' ? LIGHT_GRADIENTS[i % LIGHT_GRADIENTS.length] : DARK_GRADIENTS[i % DARK_GRADIENTS.length]} transition-all duration-700 transform ${currentIndex === i ? 'scale-100 opacity-100' : 'scale-90 opacity-10'}`}>
                  <div className="flex flex-col items-center justify-center text-center h-full pr-[22%] space-y-8">
                    <div className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border ${theme === 'dark' ? 'bg-black/40 border-white/10 text-white' : 'bg-white/60 border-black/10 text-[#002366]'}`}>
                      {fact.topic}
                    </div>
                    <h2 className={`font-black leading-tight tracking-tight px-4 ${theme === 'dark' ? 'text-white' : 'text-[#002366]'} ${fact.content.length > 70 ? 'text-lg' : 'text-xl'}`}>
                      {fact.content}
                    </h2>
              ...
              {/* Interaction Buttons (Hidden for Ads) */}
              {!fact.isAd && (
                <div className="absolute right-8 bottom-44 flex flex-col gap-6 items-center z-20">
                  <button onClick={() => setFacts(p => p.map(f => f.id === fact.id ? { ...f, liked: !f.liked } : f))} className={`p-4 rounded-[28px] backdrop-blur-3xl transition-all active:scale-75 shadow-sm ${fact.liked ? 'bg-rose-500 text-white' : 'bg-white/10 text-white'}`}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill={fact.liked ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2.5"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                  </button>
                  <button onClick={() => setShowDeckPicker(fact.id)} className={`p-4 rounded-[28px] backdrop-blur-3xl transition-all active:scale-75 shadow-sm ${fact.saved ? 'bg-sky-500 text-white' : 'bg-white/10 text-white'}`}>
                    <svg width="22" height="22" viewBox="0 0 24 24" fill={fact.saved ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2.5"><path d="M19 21l-7-4-7 4V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v16z"/></svg>
                  </button>
                  <button onClick={() => handleSpeak(fact.content)} className={`p-4 rounded-[28px] transition-all active:scale-75 shadow-sm ${isSpeaking && currentIndex === i ? 'bg-orange-500 text-white animate-pulse' : 'bg-white/10 text-white backdrop-blur-3xl'}`}>
                  ...
                  <button onClick={() => navigator.share && navigator.share({ title: 'Fact Flow', text: fact.content })} className="p-4 rounded-[28px] bg-white/10 text-white backdrop-blur-3xl shadow-sm">
                    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
                  </button>
                </div>
              )}
            </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-end justify-center bg-black/60 p-6">
          <div className={`${theme === 'light' ? 'bg-white' : 'bg-[#1a1a1a]'} w-full max-w-sm rounded-[48px] p-8 space-y-6 shadow-2xl animate-in slide-in-from-bottom duration-300`}>
            ...
            <div className="grid grid-cols-2 gap-3 max-h-60 overflow-y-auto no-scrollbar">
              ...
              }} className={`p-4 rounded-3xl ${theme === 'light' ? 'bg-slate-100' : 'bg-white/5'} text-slate-500 font-black text-[10px] uppercase text-center border-2 border-dashed border-slate-300`}>
                + NEW DECK
              </button>
            </div>
                  ...
                  {quizQuestions[currentQuizIndex].options.map((opt, i) => {
                    ...
                    return (
                      <button 
                        ...
                        className={`p-6 rounded-[32px] text-left font-bold text-sm transition-all shadow-sm flex items-center justify-between ${btnClass}`}
                      >
                        <span>{opt}</span>
                        {showAnswerFeedback && isCorrect && <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>}
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="max-w-md mx-auto space-y-10">
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
              <button 
                ...
                className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap border ${activeDeckFilter === null ? 'bg-blue-600 text-white border-transparent' : 'bg-white/5 border-slate-200/50'}`}
              >
                ALL CARDS
              </button>
              {user.decks.map(d => (
                <button 
                  ...
                  className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-nowrap border ${activeDeckFilter === d.id ? `${d.color} text-white border-transparent` : 'bg-white/5 border-slate-200/50'}`}
                >
                  {d.name}
                </button>
              ))}
        ...
        <div className={`fixed inset-0 overflow-y-auto pb-40 px-6 pt-24 ${colors.bg}`}>
          <div className="max-w-md mx-auto space-y-12">
            <div className="flex flex-col items-center">
              <button onClick={() => setShowPfpModal(true)} className="relative mb-6 group active:scale-95 transition-transform">
                <XPProgress xp={user.xp} nextLevelXp={user.nextLevelXp} size={160} strokeWidth={10} color={theme === 'dark' ? '#FF7518' : '#002366'} />
                ...
                <div className="absolute bottom-1 right-1 bg-blue-600 p-2 rounded-full border-4 border-white text-white shadow-lg">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21l-5-5m0 5l5-5"/></svg>
                </div>
              </button>
              ...
              <div className="grid grid-cols-3 gap-6">
                {[
                  ...
                  <div key={i} className="text-center">
                    ...
                    <div className="text-[8px] font-black opacity-30 uppercase tracking-widest mt-1">{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
                  ...
                  {feedbacks.map(f => (
                     <div key={f.id} className={`p-8 rounded-[48px] ${theme === 'light' ? 'bg-white shadow-sm' : 'bg-white/5'} border border-slate-200/50 space-y-6`}>
                        ...
                        <div className="flex gap-6">
                           <button onClick={() => handleLikeFeedback(f.id)} className="text-[10px] font-black opacity-40 hover:opacity-100 uppercase tracking-widest flex items-center gap-2"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M14 9V5a3 3 0 0 0-3-3l-4 9v11h11.28a2 2 0 0 0 2-1.7l1.38-9a2 2 0 0 0-2-2.3z"/></svg> {f.likes}</button>
                           <button onClick={() => setReplyTarget(f.id)} className="text-[10px] font-black opacity-40 hover:opacity-100 uppercase tracking-widest">Reply</button>
                        </div>
                        {replyTarget === f.id && (
                        ...
                        {f.replies.map(r => (
                           <div key={r.id} className="ml-6 p-5 rounded-[32px] bg-white/5 border-l-4 border-blue-500 text-xs font-bold">{r.text}</div>
                        ))}
                     </div>
                  ))}
               </div>
      ...
      {showPfpModal && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/95 p-8 animate-in fade-in duration-300">
          <div className="w-full max-w-sm flex flex-col items-center space-y-10">
            <div className="w-72 h-72 rounded-full overflow-hidden border-8 border-white shadow-2xl rotate-2">
              <img src={user.pfpUrl} className="w-full h-full object-cover" alt="Full PFP" />
            </div>
      ...
      {levelUpPopup && (
        <>
          <Confetti />
          <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/85 backdrop-blur-3xl p-10 animate-in fade-in zoom-in duration-500">
